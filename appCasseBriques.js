  ...
  if (fin === false) {
    ...
    if (y + vitesseY > canvas.height - rayonBalle) {
      ...
      if (x > barreX && x < barreX + barreWidth) {
        vitesseX = vitesseX + majorationX;
        vitesseY = vitesseY + majorationY;
        vitesseY = -vitesseY;
      } else {
        fin = true;
        affichageScore.innerHTML = `Game Over ! <br> Cliquez sur le casse-briques pour recommencer.`;
      }
    }
      ...
      if (b.statut === 1) {
        if (
          ...
          if (score === nbCol * nbRow) {
            affichageScore.innerHTML = `You win ! <br> Cliquez sur le casse-briques pour recommencer.`;
            fin = true;
          }
        }
      }
